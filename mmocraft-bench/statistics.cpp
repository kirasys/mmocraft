...
namespace bench
{
    ...
    void print_statistics()
    {
        ...
        std::cout << " - Throughput per second    : " << std::size_t(total_packet_send_count.load(std::memory_order_relaxed) / elapesd_seconds) << '\n';
        
        std::cout << "Total sended packet bytes   : " << total_packet_data_sended.load(std::memory_order_relaxed) << '\n';
        std::cout << " - Throughput per second (MB) : " << total_packet_data_sended.load(std::memory_order_relaxed) / elapesd_seconds / 1024.0 / 1024.0 << '\n';
        ...
        std::cout << " - Throughput per second    : " << total_packet_data_received.load(std::memory_order_relaxed) / elapesd_seconds / 1024.0 / 1024.0 << '\n';
    }
}